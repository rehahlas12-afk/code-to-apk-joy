...
const PalletCalcPage = () => {
  ...
  const calcPalettes = useCallback((list: PalletEntry[]) => {
    ...
    // Rule: gros palette + demi eau/lait/normal + palette normale = 3 palettes normales
    if (hasGrosEau && hasDemi && hasNormal) {
      // Already counted correctly as individual units
    }
  ...
  const addEntry = (type: PalletEntry["type"], quantity: number) => {
    setEntries(prev => [...prev, { type, quantity }]);
  };
    ...
    recognition.onresult = (event: any) => {
      ...
      if (lastResult.isFinal) {
        ...
        if (numMatch) {
          ...
          if (lower.includes("roll") || lower.includes("rôle") || lower.includes("role")) type = "roll";
          ...
          else if (lower.includes("gros")) type = "gros";
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 p-4 space-y-4 overflow-y-auto">
        ...
        >
          ...
          {listening ? "Arrêter" : "Commande vocale"}
        </button>

        {transcript && (
          <p className="text-xs text-muted-foreground bg-muted rounded-lg p-2">{transcript}</p>
        )}
        ...
        <div className="bg-card border rounded-xl p-3 space-y-2">
          <div className="flex gap-2">
            ...
            <input
              ...
              placeholder="Qté"
              className="w-20 rounded-lg border bg-background px-2 py-2 text-sm text-center"
            />
            <button
              onClick={() => {
                if (manualQty) {
          ...
          {entries.map((e, i) => (
            <div key={i} className="flex items-center justify-between bg-card border rounded-lg px-3 py-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => removeEntry(i)} className="text-destructive p-1">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
        ...
        {entries.length > 0 && (
          <div className={`rounded-xl p-4 text-center space-y-2 ${
            excess > 0 ? "bg-destructive/10 border-2 border-destructive" : "bg-accent/10 border-2 border-accent"
          }`}>
            <div className="flex items-center justify-center gap-2">
              <Calculator size={20} />
              <span className="text-2xl font-black">{total}</span>
              <span className="text-sm text-muted-foreground">/ {MAX_PALETTES} palettes</span>
            </div>
            {excess > 0 && (
        ...
        {entries.length > 0 && (
          <div className="flex gap-2">
            ...
            <button onClick={() => setEntries([])} className="bg-muted rounded-xl px-4 py-3 font-bold">
              Vider
            </button>
          </div>
        )}
